...
export const store = {
  ...
  getAttacks(limit = 100): any[] {
    return db.query('SELECT * FROM attacks ORDER BY ts DESC LIMIT ?').all(limit) as any[]
  },

  getThresholds(): Record<string, { warn: number; crit: number }> {
    const rows = db.query('SELECT id, warn, crit FROM alert_thresholds').all() as any[]
  ...
  getAttackStats(): any {
    ...
    const day = now - 86400
    return {
      last1h: (db.query('SELECT COUNT(*) as c FROM attacks WHERE ts > ?').get(hour) as any)?.c || 0,
      last24h: (db.query('SELECT COUNT(*) as c FROM attacks WHERE ts > ?').get(day) as any)?.c || 0,
